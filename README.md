...
Sanchay turns the work a professor already does — papers, mail, calendar entries, certificates, mentoring,
...
### **[ Experience Sanchay live → ](https://ggw-sih-internal-hackathon.vercel.app)**
...
![Sanchay faculty dashboard](docs/assets/screenshots/faculty-dashboard.png)

<br/>

**Contents** · [The problem](#the-problem) · [The approach](#sanchay-does-not-start-with-the-form) · [One academic memory](#one-academic-memory-every-faculty-workflow) · [Core platform](#core-platform) · [Beyond appraisal](#where-sanchay-goes-beyond-appraisal) · [Architecture](#how-sanchay-is-built) · [Engineering principles](#deterministic-first-ai-second) · [Technology](#technology-stack) · [Inside the product](#inside-sanchay) · [Built by](#built-by)
...
The original brief asks for a secure, realtime platform for faculty registration and profiles, academic activity tracking, publication discovery, evidence management, self-appraisal, administrative review, search and filtering, and report generation. Sanchay implements all of that. But building only that would produce a faster form — and the form was never the expensive part.

---

# Sanchay Does Not Start With the Form

The design decision underneath the whole product is a shift in *when* the work happens.

A traditional appraisal system activates when the appraisal opens. Everything before that moment is the professor's unaided memory. Sanchay moves the work months earlier: it captures academic signals continuously, structures them into proposed records, keeps evidence attached, and asks the professor only to confirm. By the time a form arrives, the answers already exist.

<p align="center">
  <img src="docs/diagrams/rendered/01-from-paperwork-to-academic-memory.svg" width="960" alt="Traditional faculty workflow compared with Sanchay" />
</p>
...
**Upload it once. Sanchay decides where it belongs.**
...
The same pipeline, applied to the documents that record what a professor's students went on to do — internship offers, placement letters, OJT completion certificates, research internships. Sanchay extracts the student, the company, the role, the outcome type and the dates, and stores them as structured `student_outcomes` linked to the existing student records that LOR Studio already uses.
...
# How Sanchay Is Built

<p align="center">
  <img src="docs/diagrams/rendered/11-technical-architecture.svg" width="1000" alt="Sanchay technical architecture" />
</p>
...
backend/
├── app/
│   ├── api/            19 router modules · 162 endpoints
...
frontend/
└── src/
    ├── pages/          21 page components — public, faculty and admin
    ├── components/     shared UI, modals, shell
    ├── lib/            api client · realtime · query cache · supabase
    └── styles/         brand tokens

supabase/migrations/    22 migrations · 69 tables · RLS on every one
...
Identifiers first, text second. Given an ORCID iD or an OpenAlex author ID, Sanchay queries the provider, normalises DOIs (stripping `https://doi.org/` and `doi:` prefixes, lowercasing, trimming punctuation), normalises titles to an alphanumeric form, and computes a title+year hash for records with no DOI.
...
| Drafted email reply | Review, edit, send yourself | Never sent by Sanchay |
...
- **Two independent authorization layers.** Row-level security on all 69 tables, *and* an explicit owner/institution filter in every backend query. Neither is load-bearing alone.
...
This is the security posture as built. It is not a compliance claim — Sanchay has not been through SOC 2, ISO 27001 or a formal GDPR assessment.
...
| Language model | Groq — structured extraction + vision OCR | Optional; every call site has a deterministic fallback |
...
<table>
<tr>
<td width="50%"><img src="docs/assets/screenshots/landing.png" alt="Sanchay landing page"/><br/><sub><b>The public product page</b></sub></td>
...
# What Changes With Sanchay

| Traditional workflow | With Sanchay |
| --- | --- |
...
| Check | Result |
| --- | --- |
| Backend test suite | **113 passed, 6 skipped** (`pytest backend/tests -q`) — skipped tests require an isolated live database by design |
...
| Migrations | 22, applied to the hosted project; 69 tables, RLS enabled on every one |
| Hardcoded-data audit | `qa/audit_p0.py` — **0 compulsory findings**, 8 advisory warnings (all job-polling timers) |

Known gaps, stated plainly: connecting a live Google account needs OAuth client credentials configured for the deployment; there is no embedding or semantic search anywhere; Gmail sync is on-demand rather than webhook-driven; and Any Form Assistant currently accepts `.xlsx`, with DOCX and PDF following the same pipeline but not yet wired.
...
Sanchay is built on one idea: **faculty should spend their time doing academic work, not repeatedly proving that they did it.**
